...
public class updateProjectFromTrello extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        HashMap<String, Integer> recordsMap = new HashMap<String, Integer>();
...
//                    devListForProj.add(pm.getUsername());
                    boolean change = false;
//                    System.out.println("BEFORE :: DEV LIST 1 : " + devListForProj + "  DEVLIST2 - " + devToAdd);
                    ArrayList<String> tempList = new ArrayList<String>(devListForProj);
                    devListForProj.removeAll(devToAdd);
                    devToAdd.removeAll(tempList);
//                    System.out.println("AFTER :: DEV LIST 1 : " + devListForProj + "  DEVLIST2 - " + devToAdd);
//                    if (!devToAdd.isEmpty()) {
//                        for (String devusername : devToAdd) {
...
//
//                    }
                    if (devListForProj.size() == 0 && devToAdd.size() == 0) {
                        change = false;
                    } else {
                        change = true;
                    }
                    if (change) {
                        errList.add(name + " has been modified since last sync. Please check with Trello.");
                    }

                    if (recordsMap.containsKey(name)) {
                        int value = recordsMap.get(name);
                        recordsMap.put(name, (value + 1));
                        errList.add("There are more than 2 cards on Trello named : " + name + ". Please ensure that each card is named uniquely.");
