  ...
  <script src="./bootstrap/ganttMaster.js"></script>  
    ...
    <body>
        ...
        GanttDAO gdao = new GanttDAO();
            ...
            for(String a: developer){
                ...
                for(int i=0;i<ff.size();i++){
                    if(ff.get(i).getDeveloperName().equalsIgnoreCase(deve)){
                        ...
                        if(end.getTime()<ff.get(i).getPlanend().getTime()){
                            end = ff.get(i).getPlanend();
                        }
                    }
                }
            ...
            jj = new JsonObject();
            jj.addProperty("id",count);jj.addProperty("name",a);jj.addProperty("Code","");
            jj.addProperty("level",0);jj.addProperty("status","STATUS_ACTIVE");
            jj.addProperty("canWrite",false);jj.addProperty("start",start.getTime());
            jj.addProperty("duration",duration);jj.addProperty("end",end.getTime());
            jj.addProperty("startISMilestone",true);jj.addProperty("endISMilestone",false);
            jj.addProperty("collapsed",false);jj.addProperty("hasChild", true);
            count = count -1;
            jA.add(jj);
            for(Gnatt cb: ff ){
            jj = new JsonObject();
            //long duration = (cb.getPlanend().getTime() - cb.getPlanstart().getTime())/1000*60*60*24;
            if(deve.equals(cb.getDeveloperName())){
            long duration1 = (cb.getPlanend().getTime() - cb.getPlanstart().getTime())/(1000*60*60*24);
            jj.addProperty("id",count);jj.addProperty("name",cb.getProjectName());jj.addProperty("Code","");
            jj.addProperty("level",1);jj.addProperty("status","STATUS_ACTIVE");
            jj.addProperty("canWrite",false);jj.addProperty("start",cb.getPlanstart().getTime());
            jj.addProperty("duration",duration1);jj.addProperty("end",cb.getPlanend().getTime());
            jj.addProperty("startISMilestone",true);jj.addProperty("endISMilestone",false);
            jj.addProperty("collapsed",false);jj.addProperty("hasChild", false);
            jA.add(jj);
            count = count-1;
            }
